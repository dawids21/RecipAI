...
- **meal_plan_permissions** ↔ **meal_plans**: Many-to-Many relationship through `meal_plan_permissions` join table with
  role-based access
...
- **meal_plan_entries.plan_id** → **meal_plans.id**: Foreign key relationship with ON DELETE CASCADE
- **meal_plan_entries.recipe_id** → **recipes.id**: Optional foreign key relationship with ON DELETE SET NULL
