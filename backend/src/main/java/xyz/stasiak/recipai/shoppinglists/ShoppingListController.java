...
class ShoppingListController {
    ...
    @PutMapping("/{shopping_list_id}/item/{id}")
    ResponseEntity<ShoppingListItemDto> updateItem(
        ...
        String userEmail = jwt.getClaimAsString("email");
        log.debug("Updating item {} in shopping list {} by user {} with version {}", id, shoppingListId, userEmail, version);

        ShoppingListItemDto dto = shoppingListService.updateItem(shoppingListId, id, version, request, userEmail);

        return ResponseEntity.ok(dto);
    }

    @PostMapping("/{shopping_list_id}/item/{id}/move")
    ResponseEntity<ShoppingListItemDto> moveItem(
            @PathVariable("shopping_list_id") UUID shoppingListId,
            @PathVariable UUID id,
            @RequestHeader("If-Match") Long version,
            @Valid @RequestBody MoveShoppingListItemRequest request,
            @AuthenticationPrincipal Jwt jwt) {

        String userEmail = jwt.getClaimAsString("email");
        log.debug("Moving item {} in shopping list {} by user {} with version {} to index {}", id, shoppingListId, userEmail, version, request.index());

        ShoppingListItemDto dto = shoppingListService.moveItem(shoppingListId, id, version, request.index(), userEmail);

        return ResponseEntity.ok(dto);
    }

    @PostMapping("/{shopping_list_id}/item/{id}/check")
    ResponseEntity<ShoppingListItemDto> checkItem(
            @PathVariable("shopping_list_id") UUID shoppingListId,
            @PathVariable UUID id,
            @RequestHeader("If-Match") Long version,
            @AuthenticationPrincipal Jwt jwt) {
        ...
        String userEmail = jwt.getClaimAsString("email");
        log.debug("Checking item {} in shopping list {} by user {} with version {}", id, shoppingListId, userEmail, version);

        ShoppingListItemDto dto = shoppingListService.checkItem(shoppingListId, id, version, userEmail);

        return ResponseEntity.ok(dto);
    }

    @PostMapping("/{shopping_list_id}/item/{id}/uncheck")
    ResponseEntity<ShoppingListItemDto> uncheckItem(
            @PathVariable("shopping_list_id") UUID shoppingListId,
            @PathVariable UUID id,
            @RequestHeader("If-Match") Long version,
            @AuthenticationPrincipal Jwt jwt) {
        ...
        String userEmail = jwt.getClaimAsString("email");
        log.debug("Unchecking item {} in shopping list {} by user {} with version {}", id, shoppingListId, userEmail, version);

        ShoppingListItemDto dto = shoppingListService.uncheckItem(shoppingListId, id, version, userEmail);
