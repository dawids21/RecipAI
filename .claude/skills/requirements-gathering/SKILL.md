...
description: Run a Socratic conversation to scope a programming task — surface what the system should do, what's out of scope, and how we'll know it's done — then save the result as a structured requirements document. Use this skill whenever the user describes a new feature or refactor they're considering, especially if they mention "scope", "requirements", "think through", "flesh out", or present a task description without yet asking for code. Use it even when the user's description seems complete; the skill's job is to surface the gaps. Do NOT use this skill when the user has already scoped the task and is asking for implementation help, or for narrow technical questions with no scoping component.
disable-model-invocation: true
---

# Requirements gathering

Guide the user through a Socratic-method conversation to scope a programming task (a new feature or a refactor) across seven dimensions, then write a structured requirements document the user can hand off to the next step.

This skill is a **scoping** step. It produces context, not code. The output answers *what we want to happen and why* — not *how to build it*. It feeds into subsequent steps: design docs, or directly into an implementation agent if the approach is obvious.

## The seven dimensions

Every requirements conversation should cover these areas. Not every area needs equal depth — a small feature tweak won't have much to say about integration points, and a pure refactor won't have much context to gather — but each should be touched.
...
3. Propose a filename: `docs/tasks/<YYYY-MM-DD>-<task-name>/requirements.md` with `<task-name>` in kebab-case. Ask the user to confirm or adjust.
...
When in doubt, err toward stopping. A requirements doc with a populated **Open questions** section is more useful than one dragged out by forced questioning.

## Writing the file

After the user confirms the recap:

1. Read the template at [requirements-template.md](requirements-template.md)
2. Create the directory `docs/tasks/<YYYY-MM-DD>-<task-name>/`.
3. Write `requirements.md` inside it, filling in each section from the conversation.
4. Use today's date. Use the task name the user confirmed.
5. If a section genuinely has no content (e.g., no edge cases came up for a trivial task), write a short honest note like "None identified during scoping" rather than leaving it blank or inventing filler.
...
- **Respect the user's time.** If the task is small, the scoping should be short. A tiny copy change or a one-line config tweak does not need 20 questions.
