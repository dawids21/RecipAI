...
- **`limits`** — owns per-subject usage caps for every capped resource: configuration and recorded usage in the database, override-then-default resolution read on every check (so a limit raised by SQL applies on the next request, with no restart), check-and-reserve as one indivisible conditional upsert, stock versus flow caps with lazy period restart, and the shared HTTP 429 refusal. Also serves the caller's resolved caps over `GET /limits` for a client to display, while each capped module answers for its own recorded usage on its own `/usage` route — so `limits` keeps no resource vocabulary at the HTTP edge. Holds no domain knowledge — callers pass an opaque subject and resource key (see ADR-0006). Consumed by `extraction`, `recipes`, `recipes.collections`, `shoppinglists` and `planning`; a repeatable migration rebuilds recorded usage from the owning modules' permission tables (and, for shopping-list items, from the items themselves), serving as both the rollout seed and the repair for a missed release
- **`planning`** — manages meal plans with user-based permission control (CRUD with role-based access, sharing, meal plan entries with recipe or placeholder support, an owner-scoped cap on how many plans a user may keep, automatic conversion of recipe entries to placeholders on `RecipeDeleted` event, calendar view grouped by date, shopping list generation with serving size scaling and inaccessible recipe warnings)
- **`shoppinglists`** — manages shopping lists with user-based permission control (CRUD with role-based access, per-item `baseVersion` optimistic locking on item writes — a body field on update, a query param on delete — with update covering edits, reorders, and check-state as one version-gated write; an owner-scoped cap on how many lists a user may keep, and a per-list cap on how many items a list may hold whose value is configured on the list's owner)
...
- **Usage limits** — database-backed per-subject caps resolved per request and enforced by a single conditional upsert; refusals surface as HTTP 429 with the subject's standing. Owner-scoped caps reserve on create and release on delete; the shopping-list item cap counts against the list while resolving its value from the owner. The mobile app pairs the caps read with each module's usage read to show `used / limit` at the point of action and disable it at the cap — a display and a pre-emptive block, never a second enforcement path
